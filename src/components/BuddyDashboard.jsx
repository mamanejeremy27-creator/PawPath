...
import { getBuddyPairs, getBuddyStats, acceptBuddyRequest, declineBuddyRequest, endBuddyPair, sendNudge } from "../lib/buddyMatching.js";
...
export default function BuddyDashboard() {
  ...
  const loadPairs = useCallback(async () => {
    ...
    const res = await getBuddyPairs(userId);
    if (res.error) {
      setError(true);
      setPairs([]);
    } else {
      ...
      // Fetch stats for each active buddy
      const activePairs = data.filter(p => p.status === "active");
      const statsMap = {};
      for (const p of activePairs) {
        const buddyId = p.user_a === userId ? p.user_b : p.user_a;
        const statsRes = await getBuddyStats(buddyId);
        if (!statsRes.error && statsRes.data) {
          statsMap[buddyId] = statsRes.data;
        }
      }
      setBuddyStats(statsMap);
    }
    setLoading(false);
  }, [userId]);
  ...
  const handleAccept = async (pairId) => {
    ...
    const res = await acceptBuddyRequest(pairId);
    if (!res.error) await loadPairs();
    setActionLoading(null);
  };

  const handleDecline = async (pairId) => {
    setActionLoading(pairId);
    const res = await declineBuddyRequest(pairId);
    if (!res.error) {
  ...
  const handleEnd = async (pairId) => {
    setActionLoading(pairId);
    const res = await endBuddyPair(pairId);
    if (!res.error) {
      setPairs(prev => prev.filter(p => p.id !== pairId));
    }
    setActionLoading(null);
  ...
  const handleNudge = async (toUserId, messageKey) => {
    ...
    const res = await sendNudge(toUserId, message);
    if (!res.error) {
