...
import { getReport, reportSighting } from "../lib/lostDog.js";
...
export default function ReportSighting() {
  ...
  // Load report details
  useEffect(() => {
    if (reportId) getReport(reportId).then(r => { if (r.data) setReport(r.data); });
  }, [reportId]);
  ...
  const handlePhoto = async (e) => {
    ...
    try {
      ...
      const path = await uploadPhoto("sightings", blob);
      setPhoto(path);
    } catch {
      try {
