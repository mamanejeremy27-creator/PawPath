...
import { getWeeklyLeaderboard, getAllTimeLeaderboard, getBreedLeaderboard, getUserEntry, setLeaderboardOptIn } from "../lib/leaderboard.js";
...
export default function Leaderboard() {
  const { nav, T, dogProfile, appSettings, setAppSettings, isAuthenticated, getSupaId, activeDogId } = useApp();
  const { user } = useAuth();
  ...
  const supaId = isAuthenticated ? getSupaId(activeDogId) : null;
    ...
    async function load() {
      try {
        ...
        if (tab === "weekly") {
          ...
          result = await getBreedLeaderboard(dogProfile?.breed || "");
        }
        if (cancelled) return;
        if (result.error) { setError(true); setEntries([]); }
        else setEntries(result.data || []);

        // Fetch user's own entry for highlight
        if (supaId) {
          const ue = await getUserEntry(supaId);
          if (!cancelled && ue.data) setUserEntry(ue.data);
        }
      } catch {
  ...
  const isCurrentUser = (entry) => {
    return user && entry.user_id === user.id && entry.dog_id === supaId;
  };
