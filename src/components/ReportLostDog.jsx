...
export default function ReportLostDog() {
  ...
  const handleShare = () => {
    if (!result) return;
    const text = getShareText(result, lang);
    if (navigator.share) {
      navigator.share({ title: T("lostDogAlert"), text, url: getShareUrl(result.share_token) }).catch(() => {});
    } else {
  ...
  // ── Success Screen ──
  if (result) {
    const shareUrl = getShareUrl(result.share_token);
    return (
