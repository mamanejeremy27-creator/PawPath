...
import { saveVaccination, getVaccinations, deleteVaccination, COMMON_VACCINES } from "../lib/healthTracker.js";
...
export default function VaccinationTracker() {
  ...
  useEffect(() => {
    ...
    async function load() {
      let data = [];
      if (isAuthenticated) {
        const res = await getVaccinations(activeDogId);
  ...
  const handleAdd = async () => {
    ...
    if (isAuthenticated) {
      const res = await saveVaccination(activeDogId, entry);
