...
import { saveVetVisit, getVetVisits, deleteVetVisit } from "../lib/healthTracker.js";
...
export default function VetVisitLog() {
  ...
  useEffect(() => {
    ...
    async function load() {
      let data = [];
      if (isAuthenticated) {
        const res = await getVetVisits(activeDogId);
  ...
  const handleAdd = async () => {
    ...
    if (isAuthenticated) {
      const res = await saveVetVisit(activeDogId, entry);
