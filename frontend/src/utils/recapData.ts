...
export function computeRecap({ journal, completedExercises, completedLevels, earnedBadges, totalXP, totalSessions, currentStreak, totalReviews, dogProfile, programs }) {
  ...
  // Slide 4: Best streak
  const dates = [...new Set(yearEntries.map(e => new Date((e as any).date).toDateString()))].sort((a, b) => new Date(a as string).getTime() - new Date(b as string).getTime());
  ...
  const topProgramName = Object.entries(progCounts).sort((a, b) => (b[1] as number) - (a[1] as number))[0];
  let topProgram = null;
  ...
  const topMood = Object.entries(moodCounts).sort((a, b) => (b[1] as number) - (a[1] as number))[0];
