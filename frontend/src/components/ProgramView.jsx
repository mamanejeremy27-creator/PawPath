  ...
  return (
    <div style={{ minHeight: "100vh", background: C.bg, animation: "fadeIn 0.3s ease" }}>
      <div style={{ padding: "24px 20px 32px", background: p.gradient, borderRadius: "0 0 32px 32px", position: "relative", overflow: "hidden" }}>
        ...
        <button onClick={() => nav("home", { program: null })} style={{ background: "rgba(0,0,0,0.2)", border: "none", color: "#fff", fontSize: 14, fontWeight: 600, cursor: "pointer", padding: "8px 16px", borderRadius: 20, marginBottom: 20, backdropFilter: "blur(10px)" }}>
          {rtl ? "→" : "←"} {T("back")}
        </button>
        ...
        {p.levels.map((lv, idx) => {
          ...
          return (
            <button key={lv.id} onClick={() => !locked && nav("level", { level: lv })}
              ...
              <div style={{ width: 46, height: 46, borderRadius: 14, background: done ? p.gradient : locked ? C.s2 : C.b1, display: "flex", alignItems: "center", justifyContent: "center", fontSize: done ? 18 : 16, fontWeight: 800, color: done ? "#fff" : locked ? C.t3 : C.t1, flexShrink: 0 }}>{done ? "✓" : locked ? "🔒" : idx + 1}</div>
              <div style={{ flex: 1 }}>
